...
@login_required
def doctors():
    if current_user.role != 'Doctor':  # Restrict access to doctors only
        flash("Access restricted to doctors only", "danger")
        return redirect(url_for('index'))

    # Check if the doctor already has a specialization
    existing_specialization = Doctors.query.filter_by(email=current_user.email).first()
    ...
    if request.method == "POST":
        ...
        if not email or not doctorsname or not dept:
            flash("All fields are required!", "warning")
            return render_template('doctors.html')

        # If the doctor already has a specialization, prevent adding another
        if existing_specialization:
            flash("You have already entered your specialization!", "danger")
            return redirect(url_for('doctors'))

        # Otherwise, allow them to add their specialization
        new_doctor = Doctors(email=email, doctorsname=doctorsname, dept=dept)
        db.session.add(new_doctor)
        db.session.commit()
        flash("Specialization saved successfully!", "success")

    return render_template('doctors.html', existing_specialization=existing_specialization)
